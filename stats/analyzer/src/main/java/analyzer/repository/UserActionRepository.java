...
public interface UserActionRepository extends JpaRepository<UserAction, Long> {
     ...
     * Использует default-метод для преобразования в удобную Map.
     */
    ...
    /**
     * Рассчитывает сумму весов для списка событий
     * Суммирует все actionWeight для каждого eventId.
     */
    ...
    @Query("SELECT ua.eventId, SUM(ua.actionWeight) " +
           "FROM UserAction ua " +
     ...
     *
     *
     * @Query(value = "SELECT event_id, SUM(max_weight_per_user) as total_weight " +
     *                "FROM ( " +
     *                "    SELECT event_id, user_id, MAX(action_weight) as max_weight_per_user " +
     *                "    FROM user_actions " +
     *                "    WHERE event_id IN (:eventIds) " +
     *                "    GROUP BY event_id, user_id " +
     *                ") as subquery " +
     *                "GROUP BY event_id", nativeQuery = true)
     * List<Object[]> getSumOfMaxWeightsPerUser(@Param("eventIds") List<Long> eventIds);
     */
}