...
public interface EventSimilarityRepository extends JpaRepository<EventSimilarity, Long> {
     ...
     */
    @Query("SELECT new analyzer.model.Recommendation(" +
           "    CASE WHEN es.eventA = :eventId THEN es.eventB ELSE es.eventA END, es.score" +
           ") " +
           "FROM EventSimilarity es " +
           "WHERE (es.eventA = :eventId OR es.eventB = :eventId) " +
           "AND (CASE WHEN es.eventA = :eventId THEN es.eventB ELSE es.eventA END) NOT IN :excludeIds ")
     ...
     */
    @Query("SELECT new analyzer.model.Recommendation(" +
           "    CASE WHEN es.eventA IN :sourceIds THEN es.eventB ELSE es.eventA END, MAX(es.score)" +
           ") " +
           "FROM EventSimilarity es " +
           "WHERE (es.eventA IN :sourceIds OR es.eventB IN :sourceIds) " +
           "AND (CASE WHEN es.eventA IN :sourceIds THEN es.eventB ELSE es.eventA END) NOT IN :excludeIds " +
           "GROUP BY (CASE WHEN es.eventA IN :sourceIds THEN es.eventB ELSE es.eventA END) " +
    ...
    @Query(value =
            ...
            "), " +
            "ranked_pairs AS (" +
            "    SELECT *, ROW_NUMBER() OVER (PARTITION BY primary_id ORDER BY score DESC) as rn " +
            "    FROM pairs" +
            ") " +
            "SELECT primary_id as primaryId, neighbour_id as neighbourId, score " +
            "FROM ranked_pairs " +
            "WHERE rn <= :maxNeighbours", nativeQuery = true)
